...
public class ToDoStorageTest :IDisposable
{
    ...
    public async Task UpdateToDoItemAsync_Deadline_Title_Content_Status_Success()
    {
        ...
        var status = true;
        var result = await _toDoStorage.UpdateToDoItemAsync(toDo1.Id, deadline, null, null, false);
        ...
        result = await _toDoStorage.UpdateToDoItemAsync(toDo1.Id, null, title, null, false);
        updateToDo1=await _toDoStorage.TestConnection.FindAsync<ToDo>(toDo1.Id);
        Assert.True(result);
        Assert.True(updateToDo1.Title.Equals(title));
        result = await _toDoStorage.UpdateToDoItemAsync(toDo1.Id, null, null, content, false);
        updateToDo1=await _toDoStorage.TestConnection.FindAsync<ToDo>(toDo1.Id);
        Assert.True(result);
        Assert.True(updateToDo1.Content.Equals(content));
        result = await _toDoStorage.UpdateToDoItemAsync(toDo1.Id, null, null, null, status);
        updateToDo1=await _toDoStorage.TestConnection.FindAsync<ToDo>(toDo1.Id);
        Assert.True(result);
